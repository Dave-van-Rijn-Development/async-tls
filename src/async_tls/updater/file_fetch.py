...
async def get_latest_release() -> tuple[str, list]:
    ...
    """
    async with ClientSession() as client, client.get(url, headers=_github_headers()) as response:
        data = {}
        try:
    ...
    if response.status == 200:
        ...
        raise TLSClientException(f"Failed to fetch the latest release. Status code: {response.status}")
...
async def download_and_save_asset(
    ...
    async with ClientSession() as client, client.get(asset_url, headers=headers, allow_redirects=True) as response:
        if response.status != 200:
            raise TLSClientException(f"Failed to download asset {asset_name}. Status code: {response.status}")
