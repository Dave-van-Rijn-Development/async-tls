...
class Session:
    ...
    async def execute_request(
        ...
        if not self._in_context:
            getLogger().warning('Using Session directly (without context) can lead to serious memory leaks. '
                                'Make sure to use `destroy_session` to cleanup memory after using this session, or use '
                                'Session in context to ensure memory is cleaned up automatically.')
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        if self._session_id:
            ...
            destroy_response = destroy_session(dumps(destroy_payload).encode('utf-8'))
            destroy_bytes = ctypes.string_at(destroy_response)
            destroy_response_string = destroy_bytes.decode('utf-8')
            destroy_response_object = loads(destroy_response_string)
            free_memory(destroy_response_object['id'].encode('utf-8'))
